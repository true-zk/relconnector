    ...
    def run(self, components: RuntimeComponents, *, epochs: int) -> RuntimeResult:
        ...
        def produce_batches() -> None:
            ...
            try:
                while not cancel.is_set():
                    ...
                    plans.extend(
                        plan_queue.drain(
                            self.config.feature_window_batches - 1,
                    ...
                    for batch in _assemble_many(components, features):
                        ready_queue.put(batch)
                    del features, plans, plan
            except BaseException as exc:  # noqa: BLE001 - propagate worker interrupts.
                fail(exc)
            finally:
                ready_queue.close()
        ...
        return RuntimeResult(
            ...
            pipeline={
                ...
                "ready_queue": asdict(ready_queue.stats),
...
def _assemble_many(
    components: RuntimeComponents, features: list[FeatureBatch]
) -> list[PreparedBatch]:
    method = getattr(components.assembler, "assemble_many", None)
    if callable(method):
        return cast(list[PreparedBatch], method(features))
    return [components.assembler.assemble(item) for item in features]
